...
$output="";
    ...
    $stmt = $conn->prepare("SELECT * FROM `franchisee` where status='".$status."'");
    $stmt->execute();
    ...
    if($stmt->rowCount()>0){
    	...
    	$output .= '<h2 style="text-align:center">Franchisee '.$name.' List</h2>
    ...
    foreach (($stmt->fetchAll()) as $key => $row) {
        ...
        $rDate= $rd->format('d-m-Y');
         ...
         // to get sales_manager name
        $sql4= $conn->prepare("SELECT firstname,lastname FROM `sales_manager` where sales_manager_id='".$row['reference_no']."'");
            $sql4->execute();
            $sql4->setFetchMode(PDO::FETCH_ASSOC);
            if($sql4->rowCount()>0){
                foreach (($sql4->fetchAll()) as $key4 => $row4) {
                    $sm_name=$row4['firstname']. ' ' .$row4['lastname'];
                }
            } 
    	...
    	$output .= '<tr>';
            if($name == 'Pending'){
                ...
                $output .='<td>'.$row['franchisee_id'].'</td>';
            }
            $output .= '
    ...
    if($name == 'Pending'){
        header("Content-Disposition: attachment;filename=Pending_Franchisee_List.xls");
    }else if($name == 'Registered'){
        header("Content-Disposition: attachment;filename=Registered_Franchisee_List.xls");
    }
