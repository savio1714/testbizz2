...
function getManagerId($conn,$state){
	...
	$sql2= $conn->prepare("SELECT distinct branch_manager_id  from branch_manager order by branch_manager_id");
	...
	if($sql2->rowCount()>0){
		foreach (($sql2->fetchAll()) as $key3 => $row3) {
			 ...
			 $branch_manager_id=$row3["branch_manager_id"];

			 $bmid=substr($branch_manager_id,4); //get 001
				if($bmid == $last3char){
					getManagerId($conn,$state);
				}
		}
		
		return $uid = 'BM'.$state.$last3char;
	...
	//log file
$title="Confirm Franchisee Manager";
...
$fromWhom="1";
	...
	$sql1 = "UPDATE branch_manager SET status=:status,branch_manager_id=:branch_manager_id WHERE id=:id";
	$stmt = $conn->prepare($sql1);
	$result=  $stmt->execute(array(
		':status' => $status,
		':branch_manager_id' => $uid,
		// ':deleted_date' => $today,
		':id' => $id		
	));
