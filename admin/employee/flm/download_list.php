...
$output="";
    ...
    $stmt = $conn->prepare("SELECT * FROM `sales_manager` where status='".$status."'");
    $stmt->execute();
    ...
    if($stmt->rowCount()>0){
    	...
    	$output .= '<h2 style="text-align:center">Floor Manager '.$name.' List</h2>
        <table border="1" style="text-align:center">
    	<tr>';
            if($name == 'Pending'){
                $output .= '<th>S.No</th>';
            }else if($name == 'Registered'){
                $output .='<th>Floor Manager ID</th>';
            }
    	$output .= '	
    		<th>Name</th>
    		<th>Email</th>
    ...
    foreach (($stmt->fetchAll()) as $key => $row) {
        ...
        $rDate= $rd->format('d-m-Y');
         ...
         // to get branch_manager name
        $sql4= $conn->prepare("SELECT firstname,lastname FROM `branch_manager` where branch_manager_id='".$row['reference_no']."'");
            $sql4->execute();
            $sql4->setFetchMode(PDO::FETCH_ASSOC);
            if($sql4->rowCount()>0){
                foreach (($sql4->fetchAll()) as $key4 => $row4) {
                    $bm_name=$row4['firstname']. ' ' .$row4['lastname'];
                }
            } 
    	...
    	$output .= '<tr>';
            if($name == 'Pending'){
                ...
                $output .='<td>'.$row['sales_manager_id'].'</td>';
            }
            $output .= '
    ...
    if($name == 'Pending'){
        header("Content-Disposition: attachment;filename=Pending_Floor_Manager_List.xls");
    }else if($name == 'Registered'){
        header("Content-Disposition: attachment;filename=Registered_Floor_Manager_List.xls");
    }
